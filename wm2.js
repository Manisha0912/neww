...
// Make sure the game fits properly on all screens
function adjustGameSize() {
    // Use the minimum of viewport width percentage or fixed size
    const gameSize = Math.min(window.innerWidth * 0.8, 500);
    
    // Apply size constraints to prevent overflow
    if (window.innerWidth <= 576) { // Mobile
        flex.style.width = Math.min(window.innerWidth * 0.9, 350) + 'px';
        flex.style.height = Math.min(window.innerWidth * 0.9, 350) + 'px';
    } else if (window.innerWidth <= 768) { // Small tablets
        flex.style.width = Math.min(window.innerWidth * 0.85, 400) + 'px';
        flex.style.height = Math.min(window.innerWidth * 0.85, 400) + 'px';
    } else if (window.innerWidth <= 992) { // Medium tablets
        flex.style.width = Math.min(window.innerWidth * 0.8, 450) + 'px';
        flex.style.height = Math.min(window.innerWidth * 0.8, 450) + 'px';
    } else if (window.innerWidth >= 1200) { // Large screens
        flex.style.width = Math.min(window.innerWidth * 0.7, 600) + 'px';
        flex.style.height = Math.min(window.innerWidth * 0.7, 600) + 'px';
    } else { // Default
        flex.style.width = gameSize + 'px';
        flex.style.height = gameSize + 'px';
    }
}
...
function startGame(){
    ...
    // Make sure game board is visible and properly sized
    flex.style.display = 'flex';
    adjustGameSize();
    ...
    // Start game music (with error handling)
    try {
...
window.addEventListener('resize', () => {
    ...
    // Update final score display if game is over
    if (!gameActive && timeLeft === 0) {
        responsiveFinalScore();
    }
});

// Initialize game size when page loads
